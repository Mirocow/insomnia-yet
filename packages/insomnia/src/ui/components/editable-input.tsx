...
export const EditableInput = ({
  ...
  function onDoubleClick(e: React.MouseEvent<HTMLDivElement, MouseEvent>) {
    if (e.target?.innerText === 'Base Environment') {
      setIsEditable(false);
      return;
    }
  ...
  return (
    <>
      <div
        ref={editableRef}
        className={
          `items-center truncate justify-center data-[pressed]:bg-[--hl-sm] rounded-sm text-[--color-font] hover:bg-[--hl-xs] focus:ring-inset ring-1 ring-transparent focus:ring-[--hl-md] transition-all ${isEditable ? 'hidden' : ''} ${className || 'px-2'}`
        }
