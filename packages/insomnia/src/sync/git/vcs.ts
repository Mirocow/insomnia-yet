...
export class GitVCS {
  ...
  async _hasUncommittedChanges() {
    const changes = await this.getRepositoryStatus();
  ...
  async _collectMergeConflicts(
    ...
    if (filepaths.length) {
      ...
      async function readBlob(filepath: string, oid: string) {
        const { blob, oid_1: blobId } = await git.readBlob({
          ..._baseOpts,
          oid,
          filepath,
        });
      ...
      async function readOursBlob(filepath: string) {
        return readBlob(filepath, oursHeadCommitOid);
      }

      async function readTheirsBlob(filepath: string) {
